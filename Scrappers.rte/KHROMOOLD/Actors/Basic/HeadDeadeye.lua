function Create(self)
	...
	if not string.find(self.PresetName, "Deadeye") then -- bruisers, fiends and deadeyes don't have dynamic hair
		if self.head == 2 or self.head == 6 then		-- female scrapper #1
			self.hair = CreateMOSRotating("ScrapperFaction.rte/Scrappers Hair A");
			self.hairOffset = Vector(-1,-1);
		elseif self.head == 3 or self.head == 7 then	-- female scrapper #2
